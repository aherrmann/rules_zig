...
test "zig_binary result should not contain the output base path" {
    ...
    //const binary_path = try std.fs.path.join(std.testing.allocator, &[_][]const u8{
    //    ctx.workspace_path,
    //    "bazel-bin/binary",
    //});
    //defer std.testing.allocator.free(binary_path);

    const strings_cmd = try std.fmt.allocPrint(
        std.testing.allocator,
        "strings {s}/{s} | grep /",
        .{ ctx.workspace_path, "bazel-bin/binary" },
    );
    defer std.testing.allocator.free(strings_cmd);

    var strings_proc = std.ChildProcess.init(
        &[_][]const u8{
            "sh",
            "-c",
            strings_cmd,
